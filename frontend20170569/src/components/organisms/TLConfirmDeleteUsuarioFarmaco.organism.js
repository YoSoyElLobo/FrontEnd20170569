...
const TLConfirmDeleteUsuarioFarmaco = ({trash, usuarioFarmaco, onDelete, setDeleteFarmaco}) => {

  const {t, i18n} = useTranslation();

  const handleDelete = () => {
    onDelete(usuarioFarmaco)
    return true
  }
  ...
  return (
    <>
      ...
      <Grid container justifyContent="center" alignItems="center" sx={{pt: 1}}>
        <Grid item>
          <Typography sx={{ color: 'black', fontSize: '1rem'}}>{`${i18n.language === 'es' ? usuarioFarmaco.farmaco.nombreEspanol : usuarioFarmaco.farmaco.nombreIngles }`}</Typography>
        </Grid>
      </Grid>
    </>
  )
...
export default TLConfirmDeleteUsuarioFarmaco;