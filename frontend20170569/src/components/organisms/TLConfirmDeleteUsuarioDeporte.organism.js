...
const TLConfirmDeleteUsuarioDeporte = ({trash, usuarioDeporte, onDelete, setDeleteDeporte}) => {

  const {t, i18n} = useTranslation();

  const handleDelete = () => {
    onDelete(usuarioDeporte)
    return true
  }
  ...
  return (
    <>
      ...
      <Grid container justifyContent="center" alignItems="center" sx={{pt: 1}}>
        <Grid item>
          <Typography sx={{ color: 'black', fontSize: '1rem'}}>{`${i18n.language === 'es' ? usuarioDeporte.deporte.nombreEspanol : usuarioDeporte.deporte.nombreIngles }`}</Typography>
        </Grid>
      </Grid>
    </>
  )
...
export default TLConfirmDeleteUsuarioDeporte;