...
const TLConfirmDeleteAlimento = ({trash, usuarioAlimento, onDelete, setDeleteAlimento}) => {

  const {t, i18n} = useTranslation();

  const handleDelete = () => {
    onDelete(usuarioAlimento)
    return true
  }
  ...
  return (
    <>
      ...
      <Grid container justifyContent="center" alignItems="center" sx={{pt: 1}}>
        <Grid item>
        <Typography sx={{ color: 'black', fontSize: '1rem'}}>{`${i18n.language === 'es' ? usuarioAlimento.alimento.nombreEspanol : usuarioAlimento.alimento.nombreIngles }`}</Typography>
        </Grid>
      </Grid>
    </>
  )
